...
export class Dashboard implements OnInit {
  ...
  constructor(private dashboardService: DashboardService, private cdr: ChangeDetectorRef, private router: Router) {
    ...
    forkJoin({
      cases: this.dashboardService.getCaseDetailsForDashboard(this.deptId),
      caseTypes: this.dashboardService.getCaseTypeDetailsForDashboard(this.deptId),
      oic: this.dashboardService.getOicCount(this.deptId),
      compliance: this.dashboardService.getComplianceCount(this.deptId),
      reply: this.dashboardService.getReplyCount(this.deptId),
    }).subscribe({
      next: ({ cases, caseTypes, oic, compliance, reply }: any) => {
        ...
        this.kpis = [
          ...
          { label: 'Total pending', value: this.pending_case, colorClass: 'kpi-amber', route: 'P' },
          { label: 'Pending case reply', value: this.pen_case_reply, colorClass: 'kpi-purple', route: 'OA' },
          { label: 'Court Order Compliance', value: this.disposed_case, colorClass: 'kpi-green', route: 'D' },
        ];
  ...
  navigateTo(route?: string): void {
    if (route) this.router.navigate(['/portal/ccms/adminHome/dashboard_click'], { queryParams: { type: route } });
  }
}
